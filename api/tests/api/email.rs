use crate::helpers::{spawn_app, assert_status_code};
...
async fn test_verify_email_with_used_token() {
    ...
    let token = app.get_verification_token_from_db(&credentials.email).await.unwrap();
    
    // 3. Use token once (should succeed)
    let verify_request = requests::VerifyEmail { token: token.clone() };
...
async fn test_resend_verification_email_invalidates_old_tokens() {
    ...
    let first_token = app.get_verification_token_from_db(&credentials.email).await.unwrap();
    
    // 3. Request resend verification email
    let resend_request = requests::ResendVerificationEmail {
        email: credentials.email.clone(),
    };
    app.client.resend_verification_email(&resend_request).await.unwrap();
    
    // 4. Get new verification token (should be different)
    let second_token = app.get_verification_token_from_db(&credentials.email).await.unwrap();
    ...
    let verify_request = requests::VerifyEmail { token: second_token };
    app.client.verify_email(&verify_request).await.unwrap();
...
async fn test_resend_verification_email_already_verified() {
    ...
    app.create_unverified_user(&credentials).await.unwrap();
    app.mark_user_email_verified(&credentials.username).await.unwrap();
    ...
    let token_result = app.get_verification_token_from_db(&credentials.email).await;
...
async fn test_expired_token_handling() {
    ...
    sqlx::query(
        "UPDATE tokens SET expires_at = $1 WHERE id = $2::uuid"
    )
