...
use payloads::{
    ApiError, AuctionId, AuctionRoundId, CommunityId, InviteId,
    OptionalTimestamp, Role, SiteId, SiteImageId, SpaceId, UserId,
...
pub enum StoreError {
    /// A client-facing error, serialized as JSON in the response body.
    #[error(transparent)]
    Api(#[from] payloads::ApiError),
...
fn map_space_name_unique_error(e: sqlx::Error, space_name: &str) -> StoreError {
        ...
        && db_err.is_unique_violation()
    {
        ...
        {
            return ApiError::SpaceNameNotUnique {
        ...
        && let Some(constraint) = db_err.constraint()
    {
        match constraint {
            "users_username_normalized_key" => {
                return ApiError::UsernameTaken.into();
            }
            "users_email_normalized_key" => return ApiError::EmailTaken.into(),
            _ => {}
        }
    }
    e.into()
...
pub fn span_to_interval(span: &Span) -> Result<PgInterval, StoreError> {
    span_to_interval_opt(span)
        .ok_or_else(|| ApiError::SpanTooLarge(span.to_string()).into())
}
